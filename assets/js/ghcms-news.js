...
console.log('ghcms-news.js')

// https://stackoverflow.com/questions/61553399/how-can-i-save-a-valid-image-to-github-using-their-api/61576040#61576040
...
$(document).ready(()=>{
  ...
  .then(displayPanel)
  .then(initPanel)
  .then(enableGhcmsControls)
});

async function displayPanel(){
  const $panel = $(`
<div id="ghCMS-news-panel" class="mini">
  <h2>Admin
    <a href="#" class="create-post">Nouveau</a>
  ...
  Titre: <input id="targetTitle" type="text" placeholder="Titre..."></input>
  Date: <input id="targetDate" type="text" placeholder="2021-02-17..."></input>
  <textarea placeholder="Description..."></textarea>
  New image: <input id="imgFile" type="file" name="imgFile"/>
  Image: <input id="imgName" type="text"/>
  Id: <input id="targetId" type="text" readonly></input>
  <input id="targetIndex" type="text" readonly></input>
  <input id="imgSize" type="text" readonly></input>
...
function initPanel(){
  let $panel = $('#ghCMS-news-panel');
  $panel.find('.close').on('click', disableEditorMode);
  $panel.find('.minimize').on('click', ()=>($panel.toggleClass('mini maxi')));
  $panel.find('.create-post').on('click', newPost);
  $panel.find('#targetTitle, #targetDate, textarea').on('keyup', visualizeEdit);
  $panel.find('#imgName').on('keyup', visualizeImg);
  $panel.find('#imgFile').on('click', loadImgFile);
  $panel.find('.cancel').on('click', ()=>{location=location});
  $panel.find('.validate').on('click', validateNewPost);
  console.log('init');
...
function enableGhcmsControls(){
  let $editables = $('.ghcms-news');
...
<button class="ghcms-edit-option">Edit</button>
  ...
  $editables.map((_, e) => {
    ...
    $e.find('.bloc-text').prepend($b);
  });

}

function newPost(){
  let $bloc = $($('#post-template').html());
  $bloc.attr('ghcms-feed-new', true);
  $bloc.attr('ghcms-feed-index', -1);
  const date = (new Date()).toISOString().split('T')[0];
  const placeholderId = date+'-titre';
  $bloc.prop('id', placeholderId);
  $bloc.find('h2').text('Titre');
  $bloc.find('h3').text(date);
  $bloc.find('.bloc-text > div').html('velit esse cillum dolore eu fugiat nulla pariatur. Aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem sequi nesciunt.');
  $bloc.find('img').attr('src','/assets/img/actualites/signaletique-festival-monde-de-apres.jpg');
  $('#feed').prepend($bloc);
  loadToPanel(placeholderId);
  $('#ghCMS-news-panel').removeClass('mini').addClass('maxi');
}

function loadToPanel(id){
  console.log(news);
  const $target = $('#' + id);
  console.log($target.find('h2').text())
  if ($target.length != 1){alert('Impossible de modifier cette actu, envoyer un mail à dev@chapelmele.com avec son lien et le plus d\'infos possible'); return}
  $('#ghCMS-news-panel #targetId').val(id);
  $('#ghCMS-news-panel #targetIndex').val($target.attr('ghcms-feed-index'));
  $('#ghCMS-news-panel #targetTitle').val($target.find('h2').text());
  $('#ghCMS-news-panel #targetDate').val($target.find('h3').text());
  $('#ghCMS-news-panel #imgName').val(
    ((s)=>(s.substring(s.lastIndexOf('/')+1)))($target.find('img').attr('src'))
  );
  $('#ghCMS-news-panel textarea').val(mdConverter.makeMarkdown($target.find('.bloc-text div').html()));
  $('#ghCMS-news-panel').removeClass('mini').addClass('maxi');
}
function visualizeEdit(){
  const $panel = $('#ghCMS-news-panel');
  console.log($panel.find('#targetId').val());
  let $target = $('#'+$panel.find('#targetId').val());
  $target.find('h2').text($panel.find('#targetTitle').val());
  $target.find('h3').text($panel.find('#targetDate').val());
  if ($target.attr('ghcms-feed-new')){
    console.log(slugify($target.find('h3').text()+'-'+$target.find('h2').text()))
    let newId = slugify($target.find('h3').text()+'-'+$target.find('h2').text());
    $panel.find('#targetId').val(newId);
    $target.prop('id', newId);

  }
  $target.find('.bloc-text div').html(mdConverter.makeHtml($panel.find('textarea').val()));

  console.log($target.prop('ghcms-feed-index'));
  console.log($target.attr('ghcms-feed-index'));
}

function visualizeImg(){
  const imgName = $('#ghCMS-news-panel #imgName').val();
  const targetId = $('#ghCMS-news-panel #targetId').val();
  let $targetImg = $('#'+targetId).find('img')
  $targetImg.attr('src', remote+'/news-imgs/'+imgName);
}

async function newsAsCommited(){
  ...
  const response = await octokit.request('GET /repos/{owner}/{repo}/contents/{path}', {
    ...
    console.log(r);
    newsSha = r.data.sha;
    news = JSON.parse(atob(r.data.content));
    console.log(news);
    displayPosts(news);
...
function ghcmsEncode(str){
  return btoa(encodeURI(str).replaceAll('%20',' '))
}
...
- Essayez aussi de les Limiter en Nombre, donc Relisez bien avant de valider`);
  ...
  const $panel = $('#ghCMS-news-panel');
  const id = $panel.find('#targetId').val();
  const index = $panel.find('#targetIndex').val();
  ...
  let commitMsg = '[' + ghCMSCredentials.user + '] news ' + id;

  if (env == 'dev') { 
    prompt(commitMsg, JSON.stringify(news));
  }
  if (env == 'prod') { 
    // uploadImgFile(commitMsg, newPost.picture, b64img)
    // .then(()=>{
    ...
    uploadEdits(commitMsg, news).then(()=>{
      console.log('Json uploaded successfully!');
      if (b64img){
        let commitMsg = '[' + ghCMSCredentials.user + '] news-pic ' + newPost.picture
        uploadImgFile(commitMsg, newPost.picture, b64img).then(()=>{
          location = location;
        });
      }else{
        location = location;
  ...
  document.getElementById('imgFile').onchange = function (evt) {
      ...
      if (FileReader && files && files.length) {
          ...
          fr.onload = function () {
              ...
              if (_b64img.length > 440000) { alert('image too big (>300ko)'); return}
              // if (_b64img.length > 220000) { alert('image too big (>150ko)'); return}
              b64img = _b64img

              const targetId = $('#ghCMS-news-panel #targetId').val();
              const fileName = fileNameFromPath($('#ghCMS-news-panel #imgFile').val());
              $('#'+targetId).find('img').attr('src', fr.result);
              // $('#'+targetId+' img').attr('src', remote+fileName);
              // document.getElementById('test-img').src = fr.result;
              console.log('loaded img: '+fileName);


              const imgName = $('#ghCMS-news-panel #imgName').val(targetId+fileExtention(fileName));
...
async function uploadImgFile(msg, fileName, data){
  console.log(msg)
  // const check = await octokit.request('GET /repos/{owner}/{repo}/contents/{path}', {
  //   owner: ghCMSCredentials.owner,
  //   repo: ghCMSCredentials.repo,
