...
function init(){
  ...
  $('#ghCMS-save-credentials').on('click', saveGhCMSCreedentials);
  $('#editor-mode-activate').on('click', adminModeActivate);
}

function saveGhCMSCreedentials(){
  localStorage['ghCMSCredentials-' + document.domain] = JSON.stringify({
    'owner': $('#ghCMS-owner').val(),
    'repo': $('#ghCMS-repo').val(),
    'token': $('#ghCMS-token').val(),
    'remote': $('#ghCMS-remote').val()
  });
}
